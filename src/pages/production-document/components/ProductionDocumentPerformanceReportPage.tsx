...
import { getProductionDocumentPerformanceReport } from "../lib/production-document-performance-report.actions";
import type {
...
import { PerformanceTrendTable } from "./PerformanceTrendTable";
import { PerformanceByProductTable } from "./PerformanceByProductTable";
import { PerformanceDocumentDetailsTable } from "./PerformanceDocumentDetailsTable";
import { BarChart2, Loader2, FileText, Boxes, DollarSign, Activity } from "lucide-react";

export default function ProductionDocumentPerformanceReportPage() {
  ...
  const buildParams = useCallback((): PerformanceReportParams => ({
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const result = await getProductionDocumentPerformanceReport(buildParams());
  ...
  const { summary, by_product, document_details, trend } = reportData ?? {};
    ...
    <FormWrapper>
      <div className="mb-4">
        <TitleComponent
          ...
          subtitle="Materia prima consumida vs. productos obtenidos — merma y eficiencia"
          icon="TrendingUp"
        />
      </div>
      ...
      {reportData && summary && (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {trend && trend.length > 0 && <PerformanceTrendTable data={trend} />}
            {by_product && by_product.length > 0 && (
              <PerformanceByProductTable data={by_product} />
            )}
          </div>

          {document_details && document_details.length > 0 && (
            <PerformanceDocumentDetailsTable data={document_details} />
          )}
        </>
      )}
