...
export function DebitNoteSummary({
  form,
  mode,
  isSubmitting,
  selectedSale,
  ...
  calculateTotal,
  onCancel,
}: DebitNoteSummaryProps) {
  const warehouseWatch = form.watch("warehouse_id");
  const debitNoteMotiveWatch = form.watch("debit_note_motive_id");
  ...
  // Obtener el motivo de la nota de débito seleccionado
  const selectedMotive = debitNoteMotiveWatch
    ? debitNoteReasons.find((r) => r.id.toString() === debitNoteMotiveWatch)
    : undefined;
            ...
            <div className="space-y-2 pr-2 max-h-[250px] overflow-y-auto">
              {details.length === 0 ? (
                ...
                details.map((detail, index) => {
                  ...
                  const productName = detail.concept || saleDetail?.product?.name || "Producto";
