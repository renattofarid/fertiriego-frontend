...
export function PersonRoleAssignment({
  ...
  const getRoleName = (roleId: number) => {
    return allRoles?.find(role => role.id === roleId)?.name || `Rol ${roleId}`;
  };
          ...
          <CardContent>
            <ScrollArea className="h-[400px] pr-4">
              <div className="space-y-3">
                {allRoles.map((role) => {
                  const isSelected = selectedRoles.find(sr => sr.role_id === role.id)?.status || false;
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <h4 className="font-medium">{getRoleName(roleAssignment.role_id)}</h4>
                            <p className="text-sm text-muted-foreground">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleRoleToggle(roleAssignment.role_id, false)}
